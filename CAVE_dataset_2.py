...
# 切块：一张图只随机切出一块crop_size大小的patch
...
class CAVEDataset(Dataset):
    def __init__(self, opt, type='train', batch_size=16, patch_size=64, stride=32, augment=True):
        ...
        if not os.path.exists(cache_path):
            self.raw, self.sparse_raw, self.target = [], [], [] # list
            ...
            for img_file in tqdm.tqdm(image_filenames):
                img = loadmat(img_file)['b'] # mat: [h,w,c]
                img = img.astype(np.float32)
                #img = (img-img.min()) / (img.max()-img.min()) # normalization: [0,1]
                img = img[:, :, 12:12+self.mosaic_bands]

                # generate sparse_raw iamge from GT MSI
                sparse_raw = mask_input(img, msfa_size=self.msfa_size) # sparse_raw: [h,w,c]
                # generate raw image
                raw = sparse_raw.sum(axis=2, keepdims=True) # raw: [h,w,c]
                target = img.copy() # target: [h,w,c]

                if self.type == 'train':
                    raw_patches = crop_to_patch(raw, patch_size, stride)
                    sparse_raw_patches = crop_to_patch(sparse_raw, patch_size, stride)
                    target_patches = crop_to_patch(target, patch_size, stride)
            ...
            with open(cache_path, 'wb') as f:
                pickle.dump([self.raw, self.sparse_raw, self.target], f) # python对象 -> 文件/字节流

        # if cache exist, read it
        else:
            print('Load data from cache file:', cache_path)
            with open(cache_path, 'rb') as f:
                self.raw, self.sparse_raw, self.target = pickle.load(f) # 文件/字节流 -> python对象
    ...
    def __getitem__(self, index):
        ...
        target = np.transpose(self.target[index], (2, 0, 1)) # target: [c,h,w]
        ...
        if self.type == 'train':
            ...
            if self.augment:
                ...
                target = self.data_augment(target, rotTimes, vFlip, hFlip)
