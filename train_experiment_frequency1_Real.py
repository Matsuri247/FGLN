...
if opt.dataset == 'Real':
    patch_size = 64
    stride = 64
    train_data = RealDataset(opt, type='train', patch_size=patch_size, stride=stride, augment=False)
...
def train(opt, train_loader, optimizer, scheduler, method, model, criterion, epoch, num_epochs):
    ...
    for batch in train_bar:
        input_raw, input_sparse_raw = Variable(batch[0]), Variable(batch[1])
        ...
        # mosaic loss
        mosaic_loss = criterion[1](firstcube, input_raw, is_simulate=False)
        loss = mosaic_loss.clone()

        if opt.is_freq_loss:
            # frequency: mosaic loss
            mosaic_loss_freq = criterion[2](firstcube, input_raw, need_mask=True, need_matrix=False,
                                            is_simulate=False)
        ...
        if opt.is_freq_loss:
            train_bar.set_description(
                    desc='Epoch[%d/%d] Mosaic_Loss: %.9f, Mosaic_Loss_Frequency: %.9f, Cube_Loss: %.9f, Cube_Loss_Frequency: %.9f, All_Loss: %.9f, lr: %.9f' %
                         (epoch, num_epochs, mosaic_loss.item() / batchSize_count,
                          mosaic_loss_freq.item() / batchSize_count, cube_loss.item() / batchSize_count,
                          cube_loss_freq.item() / batchSize_count, loss.item() / batchSize_count, lr))
        else:
            train_bar.set_description(
                    desc='Epoch[%d/%d] Mosaic_Loss: %.9f, Cube_Loss: %.9f, All_Loss: %.9f, lr: %.9f' %
                         (epoch, num_epochs, mosaic_loss.item() / batchSize_count, cube_loss.item() / batchSize_count,
                          loss.item() / batchSize_count, lr))

    if opt.is_freq_loss:
        logger.info('Epoch[%d/%d] Mosaic_Loss: %.9f, Mosaic_Loss_Frequency: %.9f, Cube_Loss: %.9f, Cube_Loss_Frequency: %.9f, All_Loss: %.9f, lr: %.9f' %
                    (epoch, num_epochs, mosaic_loss.item() / batchSize_count, mosaic_loss_freq.item() / batchSize_count,
                     cube_loss.item() / batchSize_count,
                     cube_loss_freq.item() / batchSize_count, loss.item() / batchSize_count, lr))
    else:
...
def validate(opt, test_loader, method, model, epoch, num_epochs):
    ...
    with torch.no_grad():
        for batch in test_bar:
            ...
            SEI = criterion_SEI(output)

            # record metrics
            metric_SEI.update(SEI.data)

            test_bar.set_description(desc='Epoch[%d/%d] SEI: %.9e' % (epoch, num_epochs, metric_SEI.avg))
    # logger
    logger.info('Epoch[%d/%d] SEI: %9e' % (epoch, num_epochs, metric_SEI.avg))
